...
| 6 | Custom API Endpoints & Redaction | OpenAI-compatible custom API support, auto-redacting secrets in diffs with [REDACTED] | M2 | COMPLETED |
| 7 | CLI Diagnostics & Pre-commit Hook | Commands `sensei stats`, `sensei log`, `sensei lint` and `.pre-commit-hooks.yaml` config | M4 | COMPLETED |
| 8 | Advanced Workflow Features | Implement `sensei pr`, `sensei squash` (Smart Rebase), and Jira/GitHub API issue context fetching | M7 | PENDING |

## Backlog / Future Ideas
- **AI Code Review (`sensei review`)**: A pre-commit or on-demand command to scan staged changes for logical bugs, missing tests, or leftover debug statements. Kept in backlog to avoid disrupting fast local dev flow.
